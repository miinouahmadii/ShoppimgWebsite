#include "market.h"
void market::open_file_csv_train() 
{
	string str;
	ifstream fin;
	fin.open("train.csv");
	...
	{	 
        string name = str;
		getline(fin, str, ',');
	    long double Good=stold(str);
		getline(fin, str, '\n');
		long double Bad=stold(str);
        good.insert(make_pair(name, Good));  
        bad.insert(make_pair(name, Bad));  
	}
    fin.close();
}
long double market::find_good_rate(string word)
{
    unordered_map<string, long double>:: iterator itr; 
    for (itr = good.begin(); itr != good.end(); itr++) 
    { 
        if(itr->first == word)
        {
            if(itr->second ==0)
                ...
                return log(itr->second);
        }
    } 
    return 0;
}
long double market::find_bad_rate(string word)
{
    unordered_map<string, long double>:: iterator itr; 
    for (itr = bad.begin(); itr != bad.end(); itr++) 
    { 
        if(itr->first == word)
        {
            if(itr->second ==0)
...
void market::check_string(string str )
{
    vector<string> words = removeDupWord(str);
    long double goods=log(num);
    long double bads=log(1-num);
    for(int i=0;i<words.size();i++)
    {
        goods += find_good_rate(words[i]);
    }
    for(int i=0;i<words.size();i++)
    {
        bads += find_bad_rate(words[i]);
    }
    int num;
    if(goods>bads)
        num = 1;
        ...
        num = 0;
    sentence.insert(make_pair(str,num));
}
void market :: open_file_csv_test()
{
    open_file_csv_train();
	string str;
	ifstream fin;
...
void market::print_percent()
{
    unordered_map<string, int>:: iterator itr; 
    int i=0;
    float correct_detected=0;
    float correct_undetected=0;
    float false_detected=0;
    float false_undetected=0;
    for (itr = sentence.begin(); itr != sentence.end(); itr++) 
    { 
        if(itr->second == test_nums[i])
        {
            if(itr->second == 1)
                correct_detected++;
            else
    ...
    Recall = (correct_detected/(correct_detected + correct_undetected))*100;
    Precision = (correct_detected/(correct_detected + false_detected))*100;
    Accuracy = ((correct_detected + false_detected)/(correct_detected+false_detected+correct_undetected+false_undetected))*100;
}
void market::open_file_class_probablities()
{
    string str;
	ifstream fin;
	fin.open("class probabilities");
    int counter=0;
    getline(fin, str, '\n');
    num = stof(str);
}
...
vector<offer*>sort_offers_by_id_or_price_descend(vector<offer*> offers,string feild)
{
    vector<int> nums;
    ...
    if(offers.size()>2)
    {
        ...
        {
            for(int i=0;i<offers.size();i++)
            nums.push_back(offers[i]->return_price());
        }
    ...
    if(offers.size()==2)
    {
        ...
        if(feild=="PriceId")
        {    if( offers[0]->return_price()==offers[1]->return_price())
            {
                if( offers[0]->return_offer_id()>offers[1]->return_offer_id())
                {    
                new_offers.push_back(offers[1]);
            ...
            if( offers[1]->return_price()>offers[0]->return_price())
            {    
                new_offers.push_back(offers[0]);
                new_offers.push_back(offers[1]);
            }
...
vector<offer*>sort_offers_by_id_or_price_ascend(vector<offer*> offers,string feild)
{
    vector<int> nums;
    vector<offer*> new_offers;
    vector<offer*> change_new_offers;
    if(offers.size()>2)
    {
        if(feild=="offerId")
        {
        ...
        for (int i=new_offers.size()-1;i>=0;i--)
        {
            change_new_offers.push_back(new_offers[i]);
        }
        return change_new_offers;
    }
    if(offers.size()==2)
    {
        if(feild=="offerId")
...
void market::show_offer_ascend()
{
    ...
    offers = sort_offers_by_id_or_price_ascend(offers,info.find("field")->second);

   for(int i=0;i<offers.size();i++)
   {
       cout<<"productId | offerId | offerUnitPrice | offerAmount"<<endl;
       cout<<offers[i]->return_offer_id()<<" | "<<" | "<<offers[i]->return_product_id()<<" | "<<offers[i]->return_price()<<" | "<<offers[i]->return_product_nums()<<endl;
   }
}
void market::show_offer_descend()
{
    vector<user*> users = HandleInput->return_v_users();
    vector<offer*> offers;
    for(int i=0;i<users.size();i++)
    ...
    offers = sort_offers_by_id_or_price_descend(offers,info.find("field")->second);
...
//     online->show_orders();
// }
void market::post_evaluateModel_command()
{
...
void market::post_commands()
{
    ...
    else if(line[1]=="evaluateModel")
        post_evaluateModel_command();
